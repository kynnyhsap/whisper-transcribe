...
def transcribe_and_save(chunk_path, output_path, accumulated_transcript):
    ...
    transcript = client.audio.transcriptions.create(
        model="whisper-1",
        file=audio_file,
        prompt=accumulated_transcript,
        response_format="verbose_json"
    )
    
    transcription_time = time.time() - start_time

    data = transcript.copy().dict()

    accumulated_transcript += data['text']
...
for i, filename in enumerate(chunk_files):
    ...
    transcription_time = transcribe_and_save(chunk_path, transcription_path, accumulated_transcript)
